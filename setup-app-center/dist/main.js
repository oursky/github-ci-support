...
async function setupAppCenter(versionSpec) {
  ...
  const binPath = import_path.default.join(stdout.trim(), ".bin");
...
async function resolveVersion(versionSpec) {
  ...
  let versions = JSON.parse(data.stdout);
  if (!Array.isArray(versions)) {
    versions = [versions];
  }
  return import_semver.default.rsort(versions)[0];
}
async function isToolAvailable(tool) {
  const path2 = await io.which(tool);
  return !!path2;
}
async function installCLI(version) {
  ...
  const tmpDir = await (0, import_promises.mkdtemp)(import_path.default.join(import_os.default.tmpdir(), "appcenter-cli-"));
  await (0, import_promises.writeFile)(import_path.default.join(tmpDir, "package.json"), "{}");
...
async function run() {
  try {
    const versionSpec = core2.getInput("cli-version", {
      required: false
    });
    await setupAppCenter(versionSpec);
  } catch (error) {
    core2.info((0, import_util.inspect)(error));
