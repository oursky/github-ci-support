...
function resolveRelease({ releases }, arch, version) {
  const archReleases = releases.filter((r) => !r.dart_sdk_arch || r.dart_sdk_arch.toLowerCase() === arch.toLowerCase());
  ...
  const matchedVersion = import_semver.default.minSatisfying(archReleases.map((r) => r.version), version);
  if (!matchedVersion) {
    throw new Error(`no matching release for version ${version}`);
  }
  return archReleases.find((r) => r.version === matchedVersion);
}
async function fetchRelease({ base_url }, release) {
  ...
  if (url.endsWith(".zip")) {
    ...
    extPath = await tc.extractTar(archivePath, void 0, "xJ");
  }
...
function setupEnv(flutterDir) {
  core.addPath(import_path.default.join(flutterDir, "bin"));
}

// src/main.ts
async function run() {
  try {
    const version = core2.getInput("flutter-version", { required: true });
    await setupFlutter(version);
  } catch (error) {
    core2.setFailed(`Action failed: ${error}`);
  }
}
run();
