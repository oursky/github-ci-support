...
async function acceptLicenses(accept) {
...
async function restorePackages(androidHome, packages) {
  for (const pkg of packages) {
    const toolPath = tc.find(packageCacheKey(pkg), dummyVersion);
    if (toolPath) {
      core.info(`Found ${pkg} in cache @ ${toolPath}`);
      const packageDir = import_path.default.join(androidHome, ...pkg.split(";"));
      core.info(`Linking ${packageDir} to ${toolPath}...`);
      await io.mkdirP(import_path.default.dirname(packageDir));
      import_fs.default.symlinkSync(toolPath, packageDir, "dir");
    }
  }
}
...
async function cachePackages(androidHome, packages) {
  for (const pkg of packages) {
    const toolPath = tc.find(packageCacheKey(pkg), dummyVersion);
    const packageDir = import_path.default.join(androidHome, ...pkg.split(";"));
    if (!toolPath && import_fs.default.existsSync(packageDir)) {
